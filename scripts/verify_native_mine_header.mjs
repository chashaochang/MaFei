...
export function validateNativeMineHeader(sources) {
  ...
  if (!/if\s*\(\s*this\.destinationOwnsTitleBar\s*\)\s*\{\s*return\s+0\s*\}/.test(spacer) ||
    !/return\s+this\.vm\.appUIState\.safeTop\s*\+\s*\(\s*this\.compactTopInset\s*\?\s*0\s*:\s*UIConstants\.ACTION_BAR_HEIGHT\s*\)/.test(spacer)) {
    throw new Error('compact MineTab header must remove only the ActionBar placeholder')
  }
