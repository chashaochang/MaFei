 ...
 */
export function copyUrl(original: url.URL): url.URL {
	return url.URL.parseURL(original.toString());
}
 ...
 */
export function hasProtocolAndPort(urlString: string, url: url.URL): boolean {
 ...
 */
export function isDefaultPort(url: url.URL): boolean {
	return !url.port
 ...
 */
export function parseUrl(input: string): url.URL {
	if (!input?.trim()) {
		throw new Error('Input is required');
	}

	return url.URL.parseURL(
		normalizeUrl(input)
	);
}
